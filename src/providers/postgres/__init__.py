"""PostgreSQL provider: run SQL queries and manage tables via async SQLAlchemy."""
...
class PostgresProvider(BaseIntegrationProvider):
    provider_key = "postgres"
    name = "PostgreSQL"
    description = "Run SQL queries and manage tables on a PostgreSQL database."
    auth_type = "token"
    base_url = ""
    timeout = 30.0
    ...
    capabilities = [
        ...
        Capability(
            ...
            params_schema={"properties": {"schema": {"type": "string"}}},
        ),
        Capability(
        ...
        Capability(
            name="insert_row",
            description="Insert a row into a table and return it.",
            params_schema={
                "required": ["table", "row"],
        ...
        Capability(
            name="update_row",
            description="Update rows matching a where clause and return them.",
            params_schema={
    ...
    def _guard(query: Any, force: bool = False) -> str:
        if not isinstance(query, str) or not query.strip():
            raise IntegrationError("query must be a non-empty string", provider="postgres")
    ...
    async def validate_connection(self) -> bool:
        ...
        if isinstance(result, list) and result:
            return list(result[0].keys())[0] is not None
        return False
    ...
    async def insert_row(self, table: str, row: dict[str, Any]) -> dict[str, Any]:
        ...
        rows = await self._execute(sql, row)
        return {"rows": rows, "rowcount": len(rows)}
    ...
    async def delete_row(self, table: str, where: dict[str, Any]) -> dict[str, Any]:
        ...
        result = await self._execute(sql, params, fetch=False)
        deleted = result.get("rowcount", 0) if isinstance(result, dict) else result.rowcount
